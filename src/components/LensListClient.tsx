...
import { filterLenses, defaultFilters, getUniqueBrands, type FilterState } from '@/lib/lenses';
...
export default function LensListClient({ lenses }: Props) {
  ...
  const filtered = useMemo(() => filterLenses(lenses, filters), [lenses, filters]);
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-6 pb-24">
        <div className="flex flex-col gap-4">
          ...
          <LensFilters filters={filters} brands={brands} onChange={setFilters} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((lens) => (
              <LensCard
                ...
                selectionDisabled={
                  compareIds.length >= MAX_COMPARE && !compareIds.includes(lens.id)
                }
